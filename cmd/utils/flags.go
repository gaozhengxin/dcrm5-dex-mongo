...
import (
	...
	coinconfig "github.com/fusion/go-fusion/crypto/dcrm/cryptocoins/config"
...
var (
	...
	// Mongo settings
	MongoEnabledFlag = cli.BoolFlag{
		Name:  "mongo",
		Usage: "Enable mongo",
	}
	// MongoSlave settings
	MongoSlaveEnabledFlag = cli.BoolFlag{
		Name:  "mongoSlave",
		Usage: "Enable mongoSlave",
	}
	// MongoIP settings
	MongoIPFlag = cli.StringFlag{
		Name:  "mongoIP",
		Usage: "mongoIP IP(default = localhost)",
		Value: "",
	}
	MatchEnabledFlag = cli.BoolFlag{
		Name:  "match",
		Usage: "Enable match",
	}
	MatchTradeFlag = cli.StringFlag{
		Name:  "trade",
		Usage: "match trade",
		Value: "",
	}
...
func SetDatadir (ctx *cli.Context,client string) {
	if path := ctx.GlobalString(DataDirFlag.Name); path != "" {
	    log.Debug("=============SetDatadir,","data dir path",path,"","==========")
	path = path + "/" + client
	cdcrm.SetDatadir(path)
	coinconfig.SetDatadir(path)
	}
}
...
func RegisterDcrmService(stack *node.Node, cfg *layer2.Config) {
	fmt.Println("====  RegisterDcrmService()  ====")
	if err := stack.Register(func(n *node.ServiceContext) (node.Service, error) {
		return layer2.DcrmNew(cfg), nil
	}); err != nil {
