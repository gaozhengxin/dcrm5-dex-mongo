...
func makeConfigNode(ctx *cli.Context) (*node.Node, gethConfig) {
	...
	cfg.Dcrm.Nodes = cfg.Node.P2P.Nodes
	...
	if ctx.GlobalBool(utils.MongoEnabledFlag.Name) {
		mongodb.Mongo = true
	}
	if ctx.GlobalBool(utils.MongoSlaveEnabledFlag.Name) {
		mongodb.Mongo = true
		mongodb.MongoSlave = true
	}
	if ctx.GlobalIsSet(utils.MongoIPFlag.Name) {
		mongodb.MongoIP = ctx.GlobalString(utils.MongoIPFlag.Name)
	}
	if ctx.GlobalIsSet(utils.MatchTradeFlag.Name) {
		mongodb.MatchTrade = ctx.GlobalString(utils.MatchTradeFlag.Name)
	}
	if ctx.GlobalBool(utils.MatchEnabledFlag.Name) {
		xprotocol.Match = true
	}
	if ctx.GlobalIsSet(utils.MatchTradeFlag.Name) {
		xprotocol.MatchTrade = strings.Split(ctx.GlobalString(utils.MatchTradeFlag.Name),",")
	}
	return stack, cfg
}
