...
import (
	...
	//TODO: dcrm p2p
	//dcrmtest "github.com/fusion/go-fusion/p2p/dcrm"

	//TODO: mongodb for web
...
var (
	...
	nodeFlags = []cli.Flag{
		...
		utils.MongoEnabledFlag,
		utils.MongoSlaveEnabledFlag,
		utils.MongoIPFlag,
		utils.MatchEnabledFlag,
		utils.MatchTradeFlag,
		configFileFlag,
	}
	...
	paillierThresholdFlags = []cli.Flag{
		...
		ORDERTestFlag = []cli.Flag{
			cli.StringFlag{Name: "ordertest", Value:"no", Usage: "order test", Destination: &ordertest},
		}
...
func init() {
	...
	app.Flags = append(app.Flags, ORDERTestFlag...)
...
func geth(ctx *cli.Context) error {
	...
	utils.SetDatadir (ctx,clientIdentifier)
	//dcrm.RestoreNodeInfo()
	node := makeFullNode(ctx)
	startNode(ctx, node)
	dcrm.InitNonDcrmChan()

	dcrm.OrderTest = ordertest//ctx.GlobalString(utils.ORDERTestFlag.Name)
	log.Debug("","dcrm.OrderTest",dcrm.OrderTest)
	xprotocol.Start()

	//TODO: dcrm test
	//fmt.Printf("dcrm.StartTest ...\n")
	//go dcrmtest.StartTest()

	//TODO: mongodb for web
	//fmt.Printf("==== mongodb.StartSync() ====\n")
	//mongodb.StartSync(utils.DataDirFlag.Name)

	//layer2 test
	if layer2Test {
		fmt.Printf("CC StartTest ...\n")
		go layer2.DcrmProtocol_startTest()
		go layer2.Xprotocol_startTest()
	        //go layer2.CC_startTest()//all
	}
