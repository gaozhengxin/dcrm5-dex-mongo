...
var (
	...
	nonceAuthMine = hexutil.MustDecode("0xaaaaaaaaaaaaaaaa") // Magic nonce number as miner.

	uncleHash = types.CalcUncleHash(nil) // Always Keccak256(RLP([])) as uncles are meaningless outside of PoW.

	diffInTurn = big.NewInt(2) // Block difficulty for in-turn signatures
	diffNoTurn = big.NewInt(1) // Block difficulty for out-of-turn signatures
	accountVIP = []common.Address{}
)
...
func (c *Clique) verifyHeader(chain consensus.ChainReader, header *types.Header, parents []*types.Header) error {
	...
	if !bytes.Equal(header.Nonce[:], nonceAuthVote) && !bytes.Equal(header.Nonce[:], nonceDropVote) {
		if !bytes.Equal(header.Nonce[:], nonceAuthMine) {
...
func (c *Clique) Prepare(chain consensus.ChainReader, header *types.Header) error {
	...
	voteAddress := common.Address{0x00}
	if number%c.config.Epoch != 0 {
		copy(header.Nonce[:], nonceAuthMine)
		c.lock.RLock()
		...
		if len(addresses) > 0 {
			copy(voteAddress[:], addresses[rand.Intn(len(addresses))][:])
			if c.proposals[voteAddress] {
	...
	copy(header.Extra[extraVote:extraVote+20], voteAddress[:]) // [0-12:info 12-31:addr]
...
func (c *Clique) Finalize(chain consensus.ChainReader, header *types.Header, state *state.StateDB, txs []*types.Transaction, uncles []*types.Header, receipts []*types.Receipt) (*types.Block, error) {
	...
	if header.Number.Uint64()%c.config.Epoch != 0 {
		state.AddBalance(header.Coinbase, calcRewards(header.Number),"FSN")
	}
...
func calcRewards(height *big.Int) *big.Int {
	...
	div2 := big.NewInt(2)
	// initial reward 2.5
	var reward = FsnBlockReward
	// every 4915200 blocks divide reward by 2, 2 Years
...
func getAccountVIP(chain consensus.ChainReader) {
	if len(accountVIP) > 0 {
		return
	}
	checkpoint := chain.GetHeaderByNumber(0)
	accountVIP = make([]common.Address, (len(checkpoint.Extra)-extraVanity-extraSeal)/common.AddressLength)
	if checkpoint != nil {
		for i := 0; i < len(accountVIP); i++ {
			copy(accountVIP[i][:], checkpoint.Extra[extraVanity+i*common.AddressLength:])
		}
		log.Info("getAccountVIP", "accountVIP", accountVIP)
	}
}
