...
func (s *Snapshot) apply(headers []*types.Header) (*Snapshot, error) {
	...
	for _, header := range headers {
		...
		// nonceAuthMine has no relation to snap.Votes
		if bytes.Equal(header.Nonce[:], nonceAuthMine) {
			continue
		}

		voteAddr := common.BytesToAddress(header.Extra[extraVote : extraVote+20])
		if voteAddr == (common.Address{}) {
			continue
		}
		...
		VoteVIP := false
		for _, addr := range accountVIP {
			if addr == signer {
				if snap.Signers[addr] == struct{}{} {
					VoteVIP = true
				}
			}
		}
		// If the vote passed, update the list of signers
		if tally := snap.Tally[voteAddr]; VoteVIP || (tally.Votes > len(snap.Signers)/2) {
			if tally.Authorize {
