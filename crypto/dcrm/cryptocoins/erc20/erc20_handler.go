...
import  (
	...
	ethhandler "github.com/fusion/go-fusion/crypto/dcrm/cryptocoins/eth"
	"github.com/fusion/go-fusion/crypto/dcrm/cryptocoins/eth/sha3"
	"github.com/fusion/go-fusion/crypto/dcrm/cryptocoins/erc20/token"
...
func (h *ERC20Handler) GetTransactionInfo(txhash string) (fromAddress string, txOutputs []ctypes.TxOutput, jsonstring string, confirmed bool, fee ctypes.Value, err error) {
	...
	log.Debug("erc20.GetTransactionInfo","hash",hash,"tx",tx,"isPending",isPending,"err1",err1)
	confirmed = !isPending
	if err1 == nil && isPending == false && tx != nil {
		...
		err = err2
		log.Debug("========ERC20 GetTransactionInfo========","msg",msg)
		contractAddress := msg.To().Hex()
		for token, addr := range Tokens {
		    log.Debug("=========ERC20 GetTransactionInfo===============","contractAddress",contractAddress,"addr",addr)
			if strings.EqualFold(contractAddress,addr) {
				jsonstring = `{"tokenType":"`+token+`"}`
				break
			}
		}
		log.Debug("========ERC20 GetTransactionInfo========","token type",jsonstring)
		//bug
		if  jsonstring == "" {
		...
		log.Debug("========ERC20 GetTransactionInfo========","data",data)
	...
	if receipterr != nil {
		...
		log.Debug("===============erc20.GetTransactionInfo,","receipt",r,"","=================")
	if r == nil {
	...
	for _, logs := range r.Logs {
		...
		log.Debug("===============erc20.GetTransactionInfo check logs","logs",logs,"ercdatanum",ercdatanum,"topics",logs.Topics,"","===============")
		for _, out := range txOutputs {
			if ercdatanum == out.Amount.String() {
				for _, top := range logs.Topics {
					...
					log.Debug("===============erc20.GetTransactionInfo check logs","aa",aa,"out.ToAddress",out.ToAddress,"","===============")
					if aa.Cmp(bb) == 0 {
						flag = true
					}
				}
...
func (h *ERC20Handler) GetAddressBalance(address string, jsonstring string) (balance ctypes.Balance, err error) {
	defer func () {
		if e := recover(); e != nil {
			err = fmt.Errorf("Runtime error: %v\n%v", e, string(debug.Stack()))
			log.Debug("==========GetAddressBalance,111111========================")
			return
		}
	} ()
/*	var args interface{}
	...
	log.Debug("=============GetAddressBalance=============","tokenType.(string)",h.TokenType,"tokenAddr",tokenAddr)
	if tokenAddr == "" {
		err = fmt.Errorf("Token not supported")
		return
	}
	...
	if err != nil {
		log.Debug("==========GetAddressBalance,2222222========================","err",err.Error())
		return
	}

	data, err := myABI.Pack("balanceOf", common.HexToAddress(address))
	if err != nil {
		log.Debug("==========GetAddressBalance,3333333========================","err",err.Error())
		return
	}
	...
	if retStruct["result"] == nil {
		...
		err = fmt.Errorf(ret)
		log.Debug("==========GetAddressBalance,44444========================","err",err.Error())
		return
	}
	...
	client, err := ethclient.Dial(url)
	if err != nil {
		log.Info("============ERC20 GetAddressBalance 111111============","Error",err)
		return
	}
	...
	if err != nil {
		log.Info("===============ERC20 GetAddressBalance 222222222==========","Error",err)
		return
	}
	...
	if err != nil {
		log.Info("===============ERC20 GetAddressBalance 3333333333==========")
		return
	}
