...
func (b *EthAPIBackend) GetEVM(ctx context.Context, msg core.Message, state *state.StateDB, header *types.Header, vmCfg vm.Config) (*vm.EVM, func() error, error) {
	state.SetBalance(msg.From(), math.MaxBig256,"FSN")
...
func (b *EthAPIBackend) SendTx(ctx context.Context, signedTx *types.Transaction) error {
	//for mangodb
	//if types.IsXvcCreateOrderTx(signedTx) {
	//	log.Debug("==== SendTx ====", "callback", "mongodb.UpdateOrderCache")
	//	var txCopy types.Transaction = *signedTx
	//	go mongodb.UpdateOrderAddCache(&txCopy)
	//}
    if types.IsXvcCreateOrderTx(signedTx) {
	if b.eth.txPool != nil {
	    //log.Info("==========EthAPIBackend.SendTx=============","order id",signedTx.Hash(),"order data",signedTx.Data())
	    go b.eth.txPool.InsToOB(signedTx)
	    b.eth.txPool.FeedTx(signedTx)
	}

	return nil
    }
    ////////
    if types.IsAddNewTradeTx(signedTx) {
	b.eth.txPool.AddNewTrade(signedTx,true)
	b.eth.txPool.FeedTx(signedTx)
	return nil
    }
    ////////
    ...
    if err != nil {
	if types.IsXvcCreateOrderTx(signedTx) && err.Error() == "success" {
	    return nil
	}
    }
