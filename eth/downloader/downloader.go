...
func (d *Downloader) Synchronise(id string, head common.Hash, td *big.Int, mode SyncMode) error {
    //log.Info("==============d.Synchronise start================","d.synchronising",d.synchronising,"id",id)
	err := d.synchronise(id, head, td, mode)
    //log.Info("==============d.Synchronise end================","d.synchronising",d.synchronising,"id",id)
	switch err {
	case nil:
	case errBusy:
	...
	case errTimeout, errBadPeer, errStallingPeer,
		...
		if d.dropPeer == nil {
		    //log.Info("==========d.Synchronise,d.dropPeer is nil============")
			// The dropPeer method is nil when `--copydb` is used for a local copy.
			// Timeouts can occur if e.g. compaction hits at the wrong time, and can be ignored
			log.Warn("Downloader wants to drop peer, but peerdrop-function is not set", "peer", id)
		} else {
			//log.Info("==========d.Synchronise,d.dropPeer============","id",id)
			d.dropPeer(id)
		}
	default:
...
func (d *Downloader) synchronise(id string, hash common.Hash, td *big.Int, mode SyncMode) error {
	...
	if !atomic.CompareAndSwapInt32(&d.synchronising, 0, 1) {
	    log.Debug("===================d.synchronise,busy error.==================","d.synchronise",d.synchronise,"errBusy",errBusy)
		return errBusy
	}
