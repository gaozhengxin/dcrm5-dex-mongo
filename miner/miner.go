...
func PendingBlock() *types.Block {
    if FSN.Miner() == nil || FSN.Miner().worker == nil {
	return nil
    }

    return FSN.Miner().worker.pendingBlock()
}

func GetCurrentBlockHeight() uint64 {
    s := BlockChain().CurrentBlock().NumberU64()
    return s
}
...
func GetDcrmAddrOutSideBalance(dcrmaddr string,cointype string) *big.Int {
   ...
   // get main net coin confirm transaction
    // ERC20 -> ETH
    // OMNI  -> BTC
    // EVT   -> EVT1
    // BEP2  -> BNB
    cointype2 := cryptocoins.GetMainNetCoin(cointype)
    hashkey := crypto.Keccak256Hash([]byte(strings.ToLower(dcrmaddr+cointype2)))
    //hashkey := crypto.Keccak256Hash([]byte(strings.ToLower(dcrmaddr+cointype)))
    signedTx,_,_,_ := rawdb.ReadTransaction(ChainDb(),hashkey)
    if signedTx == nil {
	return nil
    }

    if !types.IsDcrmConfirmAddr(signedTx) {
	return nil
    }

    from, err := types.Sender(TxPool().GetSigner(),signedTx)
    if err != nil {
	return nil
    }

    if FSN.Miner() == nil || FSN.Miner().worker == nil || FSN.Miner().worker.current == nil || FSN.Miner().worker.current.state == nil {
	return nil
    }

    return FSN.Miner().worker.current.state.GetOutSideBalance(from,cointype)
}
...
func GetDcrmAddr(dcrmaddr string,cointype string,coin string) string {
    ...
    return FSN.Miner().worker.current.state.GetDcrmAddr(from,coin)
}
...
func New(eth Backend, config *params.ChainConfig, mux *event.TypeMux, engine consensus.Engine, recommit time.Duration, gasFloor, gasCeil uint64, isLocalBlock func(block *types.Block) bool) *Miner {
	...
	dcrm.RegisterTXCallback(GetDcrmAddrOutSideBalance)
	dcrm.RegisterGetDcrmAddrCallback(GetDcrmAddr)
	core.RegisterHandleLockoutCB(HandleLockout)
